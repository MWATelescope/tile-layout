...
def getdipole(cpos=None):
  ...
  xl = vpython.box(pos=v(-width,0,(height+standoff)/2) + cpos, axis=v(0,0,1), height=boxw, width=boxw, length=height+boxw, color=vpython.color.gray(0.8), visible=False)
  xlt = vpython.box(pos=v(0,0,cpoint.z) + cpos, axis=(v(width,0,standoff)-v(-width,0,height)), height=boxw, width=boxw, color=vpython.color.gray(0.8), visible=False)
  xlb = vpython.box(pos=v(0,0,cpoint.z) + cpos, axis=(v(width,0,height)-v(-width,0,standoff)), height=boxw, width=boxw, color=vpython.color.gray(0.8), visible=False)
  xr = vpython.box(pos=v(width,0,(height+standoff)/2) + cpos, axis=v(0,0,1), height=boxw, width=boxw, length=height+boxw, color=vpython.color.gray(0.8), visible=False)

  yl = vpython.box(pos=v(0,-width,(height+standoff)/2) + cpos, axis=v(0,0,1), height=boxw, width=boxw, length=height+boxw, color=vpython.color.gray(0.8), visible=False)
  ylt = vpython.box(pos=v(0,0,cpoint.z) + cpos, axis=(v(0,width,standoff)-v(0,-width,height)), height=boxw, width=boxw, color=vpython.color.gray(0.8), visible=False)
  ylb = vpython.box(pos=v(0,0,cpoint.z) + cpos, axis=(v(0,width,height)-v(0,-width,standoff)), height=boxw, width=boxw, color=vpython.color.gray(0.8), visible=False)
  yr = vpython.box(pos=v(0,width,(height+standoff)/2) + cpos, axis=v(0,0,1), height=boxw, width=boxw, length=height+boxw, color=vpython.color.gray(0.8), visible=False)

  lna = vpython.cylinder(pos=v(0,0,cpoint.z-cylen/2) + cpos, axis=v(0,0,cylen), radius=cydia/2.0, color=color.white, visible=False)
...
def gettile(cpos=None):
  ...
  gp = vpython.box(pos=v(0,0,0) + cpos, axis=v(0,0,1), height=5.0, width=5.0, length=0.05, color=color.gray(0.5), visible=False)
  olist = [gp]
  for i in range(16):
...
def plot(tiles=None, pads=None):
  ...
  eaxis = vpython.arrow(pos=v(0,0,0), axis=v(1600,0,0), color=color.blue, shaftwidth=2, fixedwidth=True, opacity=0.2)
  eaxislabel = vpython.label(text='East', pos=v(1580,20,0), color=color.blue)

  naxis = vpython.arrow(pos=v(0,0,0), axis=v(0,1600,0), color=color.blue, shaftwidth=2, fixedwidth=True, opacity=0.2)
...
#    complist += gettile(cpos=v(tile.east, tile.north, 0.0))
    simplist.append(vpython.box(pos=v(tile.east, tile.north, 0.0), axis=v(0,0,1), height=5.0, width=5.0, length=0.2, color=color.green))
...
#    pobj.label = vpython.label(pos=pobj.pos, text=pad.name, xoffset=xoffset, yoffset=yoffset, box=False, line=False, opacity=0.2)
    pobj.label = vpython.label(pos=v(pobj.pos.x+xoffset, pobj.pos.y+yoffset, pobj.pos.z+10), text=pad.name, height=15)
    pobj.cables = {}
...
#      wmult = 3   # only useful for plots showing lightning
  ...
  if (tname is not None) and (tname in pad.inputs):
    ...
    cobj = vpython.arrow(pos=pobj.pos, axis=(tpos-pobj.pos), shaftwidth=ARROWWIDTH*wmult, fixedwidth=True, color=ccolor)
    pobj.cables[tname] = cobj
  else:
